        ...
        public static async Task SetGameAsTickerAsync()
        {
            try
            {
                ...
                using (var response = request.Content)
                {
                    ...
                    await Program.DiscordClient.SetGameAsync($"[{DateTimeOffset.FromUnixTimeSeconds(long.TryParse(Ticker.LastUpdated ?? "0", out long result) ? result : 0).ToLocalTime():HH:mm}] {Ticker.PriceJpy} JPY (hourly: {Ticker.PercentChangeOnehour}% / daily: {Ticker.PercentChangeTwentyfourhours}% / weekly: {Ticker.PercentChangeSevenDays}%)").ConfigureAwait(false);
                }
            }
            catch (Exception ex)
            {
