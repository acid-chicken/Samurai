...
namespace AcidChicken.Samurai.Tasks
{
    ...
    public static class TippingManager
    {
        ...
        public static async Task<string> InvokeMethodAsync(string method, params object[] args) => (await RpcClient.SendRequestAsync(new RpcRequest(new RpcId("1"), method, JToken.Parse($"[{string.Join(',', args.Select(x => x == null || x is bool || x is sbyte || x is byte || x is short || x is ushort || x is int || x is uint || x is long || x is ulong || x is float || x is double || x is decimal ? x.ToString() : $"\"{x}\""))}]"))).ConfigureAwait(false)).GetResult<string>();
